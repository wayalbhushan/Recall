...
export async function POST(request) {
  ...
  const shape = parsedMode === "flashcards" ? FLASHCARD_SHAPE : QUIZ_SHAPE;
  const outputKey = parsedMode === "flashcards" ? "cards" : "questions";
...
It must contain a short title string and a ${outputKey} array of exactly ${parsedCount} ${outputKey}.
${parsedMode === "quiz"
    ? "Each question needs a prompt, exactly 4 distinct strings in options, a correctIndex (integer 0-3), and a one-sentence explanation."
    : "Each card has a front (concise question or term) and a back (answer, 1-3 sentences)."}
